...
El estudio mostró que el rendimiento académico estudiantil puede
modelarse con alto desempeño a partir de variables demográficas,
hábitos de estudio, apoyo parental y promedio general. En el escenario
principal, el MLP alcanzó una exactitud de 0.8607 y un puntaje F1
macro de 0.7643, mientras que \textit{Random Forest} obtuvo el mejor
resultado global con una exactitud de 0.9331 y un puntaje F1 macro de
0.8604.
...
El análisis complementario del apéndice mostró además que, aun sin
información académica previa consolidada, es posible recuperar señal
predictiva útil mediante variables de comportamiento y acompañamiento.
Sin embargo, también evidenció un límite importante de separabilidad
entre los perfiles de desempeño más bajos y sus clases vecinas, lo
que sugiere que una caracterización temprana más precisa requeriría
atributos adicionales, por ejemplo historial académico por periodo o
indicadores contextuales más finos. En conjunto, los resultados
apuntan a dos lecturas complementarias: una de clasificación del
desempeño observado con alta exactitud, y otra de identificación
temprana del riesgo académico con valor institucional, aunque todavía
condicionada por la información disponible en el dataset.

\appendices
\phantomsection
\section{Caracterización inicial del estudiante sin información académica previa}\label{sec:apendice_temprana}

\textit{¿Qué puede saberse del perfil de un estudiante antes de contar
con información académica consolidada?} Esta es la pregunta que da
origen a este apéndice. Desde una perspectiva institucional, su
interés es claro: un sistema de alerta temprana no siempre dispone del
promedio final del periodo ni de una etiqueta de desempeño ya
construida, pero sí puede conocer variables de contexto y hábitos como
ausencias, tiempo de estudio, tutorías, apoyo parental y participación
en actividades.

\subsection{Motivación}

En el cuerpo principal del trabajo, \texttt{GPA} fue una variable
válida porque el enunciado no prohibía su uso y el objetivo era
comparar modelos sobre el conjunto completo de atributos. Sin
embargo, para fines de caracterización inicial del estudiante,
\texttt{GPA} deja de ser una señal adecuada, porque forma parte del
resultado ya observado y no de la información disponible al inicio del
seguimiento. Por ello, se abrió una línea exploratoria separada en la
que se excluyó \texttt{GPA} y se reformuló la tarea como una
aproximación a la identificación temprana de perfiles de riesgo
académico.

\subsection{Ingeniería de variables}

Para este escenario se trabajó con un conjunto base formado por
variables demográficas, de hábitos y de apoyo, sin incluir
\texttt{GPA}. Posteriormente se construyó una versión enriquecida
mediante ingeniería de variables. La intención no fue incrementar el
número de atributos de manera arbitraria, sino aproximar señales de
conducta y acompañamiento que podrían estar disponibles durante el
periodo escolar, antes de contar con un resultado académico
consolidado.
...
la ingeniería de variables sobre el mejor modelo construido sin
\texttt{GPA}. El conjunto base alcanzó una exactitud de 0.7131 y un
puntaje F1 macro de 0.5624. Al incorporar ingeniería de variables, el
rendimiento aumentó hasta 0.7604 de exactitud y 0.6039 de puntaje F1
macro. La mejora no es marginal: implica recuperar señal útil a partir
de relaciones conductuales y contextuales, incluso cuando el modelo ya
no dispone de la variable más directamente asociada con el resultado
académico final.
...
Este resultado permite una interpretación importante: cuando se
elimina la información académica ya consolidada, el problema se vuelve
claramente más difícil que el del cuerpo principal, pero no se vuelve
trivial ni imposible. La ingeniería de variables aporta valor real
porque logra recuperar parte de la estructura latente del
comportamiento estudiantil.

\subsection{Límite de separabilidad de la clase 0}

El aspecto más difícil de esta caracterización temprana fue la
...
\caption{Visualización en dos componentes principales de la separabilidad entre clases sin información académica previa.}
...
clase~1. Para dimensionar mejor este límite, también se exploraron
estrategias adicionales sobre el mismo escenario de caracterización
temprana, como se resume en la Tabla~\ref{tab:apendice_estrategias}.

\begin{table}[htbp]
\centering
\caption{Estrategias exploradas en el escenario sin información académica previa}
\scriptsize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{p{3.0cm}cc}
\toprule
Estrategia & Exactitud & Puntaje F1 macro \\
\midrule
Random Forest con ingeniería de variables & 0.7604 & 0.6039 \\
Threshold tuning sobre clase 0 & 0.7577 & 0.5974 \\
Clasificación ordinal Frank \& Hall & 0.7493 & 0.5829 \\
...
La Tabla~\ref{tab:apendice_estrategias} muestra que ninguna de estas
variantes logró superar al mejor \textit{Random Forest} con
ingeniería de variables. En otras palabras, el cuello de botella no
parece estar únicamente en la elección del algoritmo, sino en el
solapamiento real entre perfiles cercanos de bajo desempeño.
...
En consecuencia, el límite principal de este escenario no parece
estar únicamente en la arquitectura del clasificador, sino en el
nivel de solapamiento real entre perfiles de bajo desempeño y
...
institucional del problema: aun sin información académica previa, los
