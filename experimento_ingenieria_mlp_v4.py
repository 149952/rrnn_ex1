"""MLP sobre una ingenieria de variables ampliada con 12 atributos derivados.

Objetivo:
- evaluar si un MLP mejora al recibir una representacion mas rica sin GPA;
- mantener un experimento separado de las versiones previas.
"""
...
def main() -> None:
    ...
    print("Arquitectura probada: 128 -> 96 -> 64 -> 5")
    ...
    print("=== Mejor configuracion MLP sobre ingenieria_v4 ===")
    print(cfg)
    ...
    print(
        pd.DataFrame(
            [{**{k: round(v, 4) for k, v in metrics.items()}, "model": "MLP + ingenieria_v4"}]
    ...
    tuning_df.to_csv(BASE_DIR / "tuning_mlp_ingenieria_v4.csv", index=False)
    pd.DataFrame(
        [{"model": "MLP + ingenieria_v4", **{k: round(v, 4) for k, v in metrics.items()}, "best_config": str(cfg)}]
    ).to_csv(BASE_DIR / "comparacion_mlp_ingenieria_v4.csv", index=False)

    print("\nArchivos generados:")
    print(BASE_DIR / "tuning_mlp_ingenieria_v4.csv")
    print(BASE_DIR / "comparacion_mlp_ingenieria_v4.csv")
