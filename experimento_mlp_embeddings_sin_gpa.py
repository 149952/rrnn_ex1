"""MLP con embeddings para variables categoricas en el escenario sin GPA.

Objetivo:
- medir si embeddings mejora la representacion categorica en un escenario
  mas predictivo y de alerta temprana;
- comparar contra los resultados sin GPA ya obtenidos con otras variantes.
"""
...
CONTINUOUS = ["Age", "StudyTimeWeekly", "Absences"]
CATEGORICAL = [
    "Gender",
    "Ethnicity",
    "ParentalEducation",
...
def build_embedding_mlp(
    ...
    model = keras.Model(inputs=inputs, outputs=out, name="mlp_embeddings_sin_gpa")
    model.compile(
...
def main() -> None:
    ...
    print(f"Continuas sin GPA: {len(CONTINUOUS)} | Categoricas con embedding: {len(CATEGORICAL)}")
    ...
    print("\n=== Mejor configuracion MLP con embeddings sin GPA ===")
    print(cfg)
    ...
    print(
        pd.DataFrame(
            [{**{k: round(v, 4) for k, v in metrics.items()}, "model": "MLP + embeddings sin GPA"}]
    ...
    tuning_df.to_csv(BASE_DIR / "tuning_mlp_embeddings_sin_gpa.csv", index=False)
    pd.DataFrame(
        [{"model": "MLP + embeddings sin GPA", **{k: round(v, 4) for k, v in metrics.items()}, "best_config": str(cfg)}]
    ).to_csv(BASE_DIR / "comparacion_mlp_embeddings_sin_gpa.csv", index=False)

    print("\nArchivos generados:")
    print(BASE_DIR / "tuning_mlp_embeddings_sin_gpa.csv")
    print(BASE_DIR / "comparacion_mlp_embeddings_sin_gpa.csv")
