"""MLP con una capa menos sobre ingenieria_v1 sin GPA.

Objetivo:
- probar una variante intermedia entre el MLP base y el MLP mas profundo;
- comparar si reducir una capa mejora la generalizacion.
"""
...
def main() -> None:
    ...
    print("=== Mejor configuracion MLP intermedio sobre ingenieria_v1 ===")
    print(cfg)
    ...
    print(
        pd.DataFrame(
            [{**{k: round(v, 4) for k, v in metrics.items()}, "model": "MLP intermedio + ingenieria_v1"}]
    ...
    tuning_df.to_csv(BASE_DIR / "tuning_mlp_ingenieria_v1_intermedio.csv", index=False)
    pd.DataFrame(
        [
            {
                "model": "MLP intermedio + ingenieria_v1",
                **{k: round(v, 4) for k, v in metrics.items()},
                "best_config": str(cfg),
            }
        ]
    ).to_csv(BASE_DIR / "comparacion_mlp_ingenieria_v1_intermedio.csv", index=False)

    print("\nArchivos generados:")
    print(BASE_DIR / "tuning_mlp_ingenieria_v1_intermedio.csv")
    print(BASE_DIR / "comparacion_mlp_ingenieria_v1_intermedio.csv")
