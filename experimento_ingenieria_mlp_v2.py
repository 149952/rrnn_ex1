"""MLP mas profundo sobre el escenario de ingenieria de variables sin GPA.

Objetivo:
- probar una version con dos capas ocultas adicionales;
- comparar contra el MLP previo y contra Random Forest con ingenieria_v1.
"""
...
def main() -> None:
    ...
    print("Arquitectura probada: 128 -> 96 -> 64 -> 32 -> 5")
    ...
    print("=== Mejor configuracion MLP profundo sobre ingenieria_v1 ===")
    print(cfg)
    ...
    print(
        pd.DataFrame(
            [{**{k: round(v, 4) for k, v in metrics.items()}, "model": "MLP profundo + ingenieria_v1"}]
    ...
    tuning_df.to_csv(BASE_DIR / "tuning_mlp_ingenieria_v1_profundo.csv", index=False)
    pd.DataFrame(
        [
            {
                "model": "MLP profundo + ingenieria_v1",
                **{k: round(v, 4) for k, v in metrics.items()},
                "best_config": str(cfg),
            }
        ]
    ).to_csv(BASE_DIR / "comparacion_mlp_ingenieria_v1_profundo.csv", index=False)

    print("\nArchivos generados:")
    print(BASE_DIR / "tuning_mlp_ingenieria_v1_profundo.csv")
    print(BASE_DIR / "comparacion_mlp_ingenieria_v1_profundo.csv")
