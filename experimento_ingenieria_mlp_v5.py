"""MLP con 5 capas ocultas y variacion de dropout sobre ingenieria ampliada.

Supuesto de trabajo:
- usar el escenario de ingenieria mas rico sin GPA (12 base + 12 derivados);
- probar si una arquitectura mas profunda mejora cuando se regulariza mejor.
"""
...
def build_ingenieria_v4(df: pd.DataFrame) -> pd.DataFrame:
    base_cols = [
        "Age",
        "Gender",
        "Ethnicity",
...
def main() -> None:
    set_seed()
    df = load_df()
    x = build_ingenieria_v4(df)
    y = df[TARGET].to_numpy()
    num_classes = int(df[TARGET].nunique())

    print(f"Dataset: {len(df)} registros")
    print("Atributos base: 12")
    print("Atributos derivados: 12")
    print(f"Total de atributos: {x.shape[1]}")
    print("Arquitectura probada: 160 -> 128 -> 96 -> 64 -> 32 -> 5")
    print("Barrido de dropout: 0.1, 0.2, 0.3, 0.4")
    ...
    print("=== Mejor configuracion MLP 5 capas sobre ingenieria_v4 ===")
    print(cfg)
    ...
    print(
        pd.DataFrame(
            [{**{k: round(v, 4) for k, v in metrics.items()}, "model": "MLP 5 capas + ingenieria_v4"}]
    ...
    tuning_df.to_csv(BASE_DIR / "tuning_mlp_ingenieria_v5.csv", index=False)
    pd.DataFrame(
        [
            {
                "model": "MLP 5 capas + ingenieria_v4",
                **{k: round(v, 4) for k, v in metrics.items()},
                "best_config": str(cfg),
            }
        ]
    ).to_csv(BASE_DIR / "comparacion_mlp_ingenieria_v5.csv", index=False)

    print("\nArchivos generados:")
    print(BASE_DIR / "tuning_mlp_ingenieria_v5.csv")
    print(BASE_DIR / "comparacion_mlp_ingenieria_v5.csv")
